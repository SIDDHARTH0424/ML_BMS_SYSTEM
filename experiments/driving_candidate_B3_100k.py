"""
Track B — Candidate B3, regen-weight bump at 100,000 steps/seed.

Single-variable isolation experiment: at 100,000 steps/seed, Candidate
B2 (w_regen_recovery=1.2, w_energy_cost=0.2, w_tracking_error=1.5,
ent_coef=0.010) got very close but did not clear the strict gate
(cross-seed mean 129.24 vs 129.16 gate; only 1/3 seeds cleared both
Wh/km and per-cycle conditions -- see audit/driving_Cand_B2_100k_report.md).
Per Absolute Rule 8 (change one mechanism at a time), this candidate
changes ONLY w_regen_recovery (1.2 -> 1.4) at the same 100k budget and
ent_coef=0.010 -- testing whether a slightly stronger regen incentive
closes the remaining gap for seeds 21/42 without needing a longer budget.

Does NOT overwrite Cand_B1/Cand_B2 (50k) or Cand_B2_100k results.
New experiment name: Cand_B3_100k.

Usage:
    python -m experiments.driving_candidate_B3_100k
"""
...
SEEDS = [7, 21, 42]
TIMESTEPS = 100_000
CAND_NAME = "Cand_B3_100k"
CAND_WEIGHTS = {"w_regen_recovery": 1.4, "w_energy_cost": 0.2, "w_tracking_error": 1.5}
ENT_COEF = 0.010
